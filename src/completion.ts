...
export function showPicker(candidates: string[], onSelect: (chosen: string) => void, onCancel: () => void, onHistory?: () => void): void {
  ...
  function buildLeft(): string {
    ...
    if (cmds > 0) parts.push(`${cmds} item${cmds === 1 ? "" : "s"}`);
  ...
  function drawContent(): void {
    const start = NR + 2; const p = pr(); const c = cw(); const v = vis();
    let out = "";
  ...
  stdin.on("data", (key: string) => {
    ...
    else if (key === "\u001b[F") idx = candidates.length - 1;
...
export function tokenizeLine(line: string): string[] {
  const tokens: string[] = [];
  let current = ""; let inDouble = false; let inSingle = false;
